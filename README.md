...
The **processor** is a 5-stage in-order pipeline: `FETCH → DECODE → EXECUTE → MEMORY → WRITE_BACK`. All five stages operate concurrently each clock cycle, giving an ideal CPI of 1. A simple 3-state FSM (`HALT → INIT → RUN`) controls startup and halting; once in `RUN` all pipeline stages advance together.
...
- *Data hazards (RAW)*: a `conflict_checker` module compares the source registers of the instruction in Decode against the destination registers of instructions in Execute, Memory, and Write-Back. If a match is found the Decode stage stalls (inserts a NOP into the D/E register) until the producing instruction writes back.
...
| `processor3.sv` | 5-stage pipelined CPU core with prefetch buffer and hazard handling |
| `conflict_checker.sv` | RAW data hazard detector — compares source vs destination registers |
