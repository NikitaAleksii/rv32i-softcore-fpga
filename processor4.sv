...
module processor #(
    ...
    // Conflict registers
    logic conflict_de, conflict_dm;
    logic [31:0] d_forward_jumpr_target;
    logic [31:0] d_forward_adjusted_load_addr, d_forward_adjusted_store_addr;
    logic [31:0] d_forward_rs1_data, d_forward_rs2_data;
    logic [31:0] e_forward_rs1_data, e_forward_rs2_data;

    forwarder forwarder_inst(
    .d_instruction(d_effective_instruction),
    .e_instruction(e_effective_instruction),
    .m_instruction(m_effective_instruction),
    .w_instruction(w_effective_instruction),
    .e_reg_write_data(e_reg_write_data),
    .m_reg_write_data(em_reg_write_data),
    .w_reg_write_data(mw_reg_write_data),
    .w_load_data(w_load_data),
    .d_rs1_data(registers[d_rs1]),
    .d_rs2_data(registers[d_rs2]),
    .e_rs1_data(de_rs1_data),
    .e_rs2_data(de_rs2_data),
    .d_Iimm(d_Iimm),
    .d_Simm(d_Simm),
    .conflict_de,
    .conflict_dm,
    .d_forward_jumpr_target,
    .d_forward_adjusted_load_addr,
    .d_forward_adjusted_store_addr,
    .d_forward_rs1_data,
    .d_forward_rs2_data,
    .e_forward_rs1_data,
    .e_forward_rs2_data
    ...
    logic [31:0] e_reg_write_data;
    always_comb begin
        if (e_isJAL || e_isJALR) begin
            e_reg_write_data = de_PC + 4;
        end else if (e_isLUI) begin
            e_reg_write_data = e_Uimm;
        end else if (e_isAUIPC) begin
            e_reg_write_data = de_PC + e_Uimm;
        end else if (e_isALUreg || e_isALUimm) begin
            e_reg_write_data = e_aluOut;
        end else if (e_isCSR_RS) begin
            e_reg_write_data = e_csr_data;
        end else begin
            e_reg_write_data = 32'b0;
        end
    end

    // Load-use hazard -- need to stall
    logic conflict_d_emw;
    assign conflict_d_emw = (conflict_de && e_isLoad) || (conflict_dm && em_isLoad);
    ...
    alu #(
        ...
        .isBranch(e_isBranch),
        .rs1_data(e_forward_rs1_data),
        .rs2_data(e_forward_rs2_data),
        .Iimm(e_Iimm),
        .funct3(e_funct3),
        .funct7(e_funct7),
        .isALUreg(e_isALUreg),
    ...
    assign e_store_word = e_forward_rs2_data;
...
`endif 
                    if (!conflict_d_emw && !pb_empty) begin
                        ...
                        de_jumpr_target <= d_forward_jumpr_target;
                        de_branch_target <= d_PC + d_Bimm;

                        de_adjusted_load_addr <= d_forward_adjusted_load_addr;
                        de_adjusted_store_addr <= d_forward_adjusted_store_addr;

                        de_PC <= d_PC;

                        de_rs1_data <= d_forward_rs1_data;
                        de_rs2_data <= d_forward_rs2_data;
