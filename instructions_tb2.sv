...
module instructions_tb ();
    ...
    task automatic step(int n = 1);
        logic [63:0] initial_retired;
        repeat (n) begin
            initial_retired = soc_inst.processor_inst.instructions_retired;
            @(posedge clock);
            while (soc_inst.processor_inst.instructions_retired === initial_retired) begin
                @(posedge clock);
            end
        end
    endtask
    ...
    initial begin
        ...
        expect_mem(WORD_0, 32'h0000_007E, "sw x11, 0(x21)");
        expect_mem(WORD_4, 32'h002A_007E, "sb x12, 6(x21)");
